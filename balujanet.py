...
from models.BalujaNet import balujanet
...
mkdirs('results')
logger_name = 'balujanet_trained_ON_' + c.data_name_train
logger_info(logger_name, log_path=os.path.join('results', logger_name+'.log'))
logger = logging.getLogger(logger_name)
logger.info('#'*50)
logger.info('model: balujanet')
...
if c.mode == 'test':
        ...
        for idx, data in enumerate(stream):
            ...
            if c.save_processed_img == True:
                ...
                for cur_dir in super_dirs:
                    mkdirs(os.path.join(c.IMAGE_PATH, 'balujanet', c.data_name_test, cur_dir))    
                image_name = '%.4d.' % idx + c.suffix
                save_image(cover, os.path.join(c.IMAGE_PATH, 'balujanet', c.data_name_test, super_dirs[0], image_name))
                save_image(secret, os.path.join(c.IMAGE_PATH, 'balujanet', c.data_name_test, super_dirs[1], image_name))
                save_image(stego, os.path.join(c.IMAGE_PATH, 'balujanet', c.data_name_test, super_dirs[2], image_name))
                save_image(secret_rev, os.path.join(c.IMAGE_PATH, 'balujanet', c.data_name_test, super_dirs[3], image_name))
                save_image(cover_resi, os.path.join(c.IMAGE_PATH, 'balujanet', c.data_name_test, super_dirs[4], image_name))
                save_image(secret_resi, os.path.join(c.IMAGE_PATH, 'balujanet', c.data_name_test, super_dirs[5], image_name))
    ...
    for epoch in range(c.epochs):
        ...
        for batch_idx, data in enumerate(stream):
            ...
            loss =  c.baluja_lambda_S * S_loss + c.baluja_lambda_R * R_loss
