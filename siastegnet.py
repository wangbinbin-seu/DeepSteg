...
from models.SiaStegNet import KeNet
...
if c.mode == 'test':
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, labels) in enumerate(stream):
            ...
            inputs, labels = preprocess_data(inputs*255, labels, False)

            # inputs = inputs.to(device, dtype=torch.float)
            # labels = labels.to(device, dtype=torch.long)

            ################## forward ####################
            outputs, feats_0, feats_1 = model(*inputs)
    ...
    # optimizer = torch.optim.Adamax(model.parameters(), lr=1e-3, eps=1e-8, weight_decay=1e-4) # default opptimizer
    ...
    for epoch in range(c.epochs):
        ...
        for batch_idx, train_batch in enumerate(stream):
            ...
            loss = loss_fn1(outputs, labels) + \
                   alpha * loss_fn2(feats_0, feats_1, labels)
