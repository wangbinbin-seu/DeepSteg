...
from models.ZhuNet import Zhu_Net
...
model_save_dir = os.path.join('checkpoints', 'ZhuNet')
results_save_dir = os.path.join('results', 'ZhuNet')
...
model = Zhu_Net().to(device)

if c.mode == 'test':

    test_loader = get_test_loader(c.test_data_dir, c.test_batch_size)
    model.load_state_dict(torch.load(c.pre_trained_zhunet_path))
